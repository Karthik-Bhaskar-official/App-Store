...
class AppStore extends Component {
  state = {
    category: tabsList[0].displayText,
    searchQuery: '',
  }

  handleClick = displayText => {
    this.setState({category: displayText})
  }
        ...
        <ul className="un-order-list">
          {tabsList.map(tab => (
            <TabItem
              ...
              isTrue={tab.displayText.toLowerCase() === category.toLowerCase()}
            />
          ))}
        </ul>
        <ul className="app-ul">
          {filteredList.map(app => (
            <AppItem name={app} key={app.appId} />
          ))}
        </ul>
      </div>
    )
